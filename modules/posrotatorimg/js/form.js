      ...
      var dropzoneOptions = {
        ...
        success: function(file, response) {
          ...
          if (response.cover === 1) {
            ...
            rotator.updateDisplayRotator(response.id);
          }
        },
...
var rotator = (function(){
  ...
  var id_product = $('#form_id_product').val();
  var id_shop1 = $('.shop.active > a').attr('href');
  var id_shop = 0;
  if(id_shop1){
  matches = id_shop1.match(/\d+$/);
  if(matches){
    id_shop = matches[0];
  }
  }else{
  ...
  return {
    'loaddataform': function(id){
        $.ajax({
          ...
          success: function(json) { 
            ...
            if(data.id_image == id) {
              $('#form_image_rotator').prop('checked', true);
            }
          },
    ...
    'getidrotator': function(){
        $.ajax({
          ...
          success: function(json) { 
            data = JSON.parse(json);
            rotator.updateDisplayRotator(data.id_image);
          },
        });
    },
    'test': function(id){
      formZoneElems.find('#form_image_rotator').on('click', function(){
        if($('#form_image_rotator').is(':checked')){
          var idImage = id;
        }else{
          var idImage = 0; 
        }
        $.ajax({
            type: 'POST',
            url:'../../../../modules/posrotatorimg/ajax.php',
            data: 'img_id='+idImage+'&id_product='+id_product+'&action=1'+'&shop_id='+id_shop,
            dataType: 'json',
            success: function(json) {
              showSuccessMessage('Update successful');
              rotator.updateDisplayRotator(idImage)   ;
            },
        });
      })
    },
    'updateDisplayRotator': function(id_image) {
      $('#product-images-dropzone .dz-preview .isRotator').remove();
      $('#product-images-dropzone .dz-preview[data-id="' + id_image + '"]')
        .append('<div class="iscover isRotator">Rotator</div>');
    },
  };
})();
/**
